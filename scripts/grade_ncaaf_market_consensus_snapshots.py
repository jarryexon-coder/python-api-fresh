#!/usr/bin/env python3
"""Attach final NCAAF player results to forward market snapshots."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Grade saved NCAAF player-prop observations.")
    ...
    from api.prediction_ledger import grade_ncaaf_market_consensus
    with flask_app.test_request_context(
        "/api/prediction-ledger/snapshots/ncaaf/grade-market-consensus?" + urlencode({"date": args.date}),
        method="POST", headers={"X-Prediction-Import-Key": secret},
    ):
        response = grade_ncaaf_market_consensus()
