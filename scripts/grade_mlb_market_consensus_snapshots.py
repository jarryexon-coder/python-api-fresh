#!/usr/bin/env python3
"""Settle forward MLB market-consensus snapshots after final box scores exist."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Attach MLB final player results to forward market snapshots.")
    ...
    from api.prediction_ledger import grade_mlb_market_consensus
    with flask_app.test_request_context(
        "/api/prediction-ledger/snapshots/mlb/grade-market-consensus?" + urlencode({"date": args.date}),
        method="POST",
        headers={"X-Prediction-Import-Key": secret},
    ):
        response = grade_mlb_market_consensus()
