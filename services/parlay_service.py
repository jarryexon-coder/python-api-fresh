...
class ParlayService2026:
    ...
    def calculate_correlation(legs: List[Dict]) -> float:
        ...
        # QB + WR correlation
        if any(l["market"] == "Passing TDs" for l in legs) and any(
            l["market"] == "Receiving TDs" for l in legs
        ):
            correlation_score += 0.15
        # Star player + team win
        if any(l.get("is_star", False) for l in legs) and any(
            l["market"] == "Moneyline" for l in legs
        ):
