...
class SuggestionWorker:
    ...
    async def generate_nba_suggestions(self):
        ...
        await self.redis_client.setex(
            "parlay:suggestions:nba", self.cache_ttl, json.dumps(suggestions)
        )
    ...
    async def generate_nhl_suggestions(self):
        ...
        await self.redis_client.setex(
            "parlay:suggestions:nhl", self.cache_ttl, json.dumps(suggestions)
        )
    ...
    async def generate_cross_sport_suggestions(self):
        ...
        suggestions = self.ai_service.generate_cross_sport_parlays(
            nba_games[:2], nhl_games[:2]
        )

        await self.redis_client.setex(
            "parlay:suggestions:cross", self.cache_ttl, json.dumps(suggestions)
        )
