...
class UnifiedNBADataPipeline:
    ...
    def fetch_sleeper_data(self) -> Dict[str, Any]:
        ...
        league = requests.get(
            f"{self.sleeper_base}/league/{self.sleeper_league_id}"
        ).json()
        rosters = requests.get(
            f"{self.sleeper_base}/league/{self.sleeper_league_id}/rosters"
        ).json()
    ...
    def fetch_tank01_data(self) -> Dict[str, Any]:
        ...
        adp_resp = requests.get(
            f"https://{self.tank01_host}/getNBAADP", headers=headers
        )
        ...
        projections = (
            proj_resp.json().get("body", {}).get("playerProjections", {})
            if proj_resp.ok
            else {}
        )
        ...
        inj_resp = requests.get(
            f"https://{self.tank01_host}/getNBAInjuryList", headers=headers
        )
    ...
    # ------------------- Merging -------------------
    def merge_players(
        self, sleeper_players: Dict, tank01_data: Dict, dk_salaries: Dict
    ) -> List[Dict]:
        """
            ...
            unified.append(
                {
                    ...
                    "injury_status": (
                        "Injured"
                        if tank01_match and inj_map.get(tank01_match)
                        else "Healthy"
                    ),
                    "adp": (
                        adp_map.get(tank01_match, {}).get("overallADP")
                        if tank01_match
                        else None
                    ),
                    "projection": (
                        proj_map.get(tank01_match, {}).get("fantasyPoints")
                        if tank01_match
                        else None
                    ),
...
if __name__ == "__main__":
    ...
    pipeline = UnifiedNBADataPipeline(
        ...
        rapidapi_key=os.getenv(
            "RAPIDAPI_KEY", "cdd1cfc95bmsh3dea79dcd1be496p167ea1jsnb355ed1075ec"
        ),
    )
